...
其中\lstinline|state.getAndSet()|实现的结果和上面讲的TAS操作相同。它会将目标的值设置为参数里的值，并返回目标原来的值。如果当前锁空闲(值为0)，有线程要请求锁，那么\lstinline|state.getAndSet(true)|会把锁置为1，并且返回false。如果当前锁被占用(值为1)，那么\lstinline|state.getAndSet(true)|会返回1，导致线程进入自旋等待，直到锁被释放。
